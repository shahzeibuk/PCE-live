...
export function PartnersShowcase({
  ...
  return (
    <section
      className={`border-y border-slate-200 dark:border-slate-800 bg-slate-50/80 dark:bg-slate-900/40 ${className}`}
    >
      <div className="container px-4 py-12 md:py-16">
        <div className="max-w-3xl mx-auto text-center mb-10">
          ...
          {subtitle ? (
            <p className="mt-3 text-slate-600 dark:text-slate-400 text-sm md:text-base">{subtitle}</p>
          ) : null}
        </div>
        <ul className="grid grid-cols-2 md:grid-cols-4 gap-6 md:gap-8 max-w-5xl mx-auto list-none p-0 m-0">
          {PARTNER_LOGOS.map((p) => (
            <li
              key={p.name}
              className="flex items-center justify-center rounded-lg border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950 px-4 py-6 shadow-sm"
            >
              <Image
                src={p.src}
                alt={p.name}
